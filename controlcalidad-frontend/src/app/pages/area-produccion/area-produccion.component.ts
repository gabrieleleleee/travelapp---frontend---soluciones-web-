...
export class AreaProduccionComponent implements OnInit {
  ...
  private readonly service = inject(AreaProduccionService);
  ...
  guardar(): void {
    ...
    const datos = this.formulario.value as AreaProduccion;
    if (this.editando()) {
  ...
  editar(item: AreaProduccion): void {
    // ✅ signal.set() para actualizar estado de edición
    this.editando.set(true);
    this.idSeleccionado.set((item as any).idArea);
