...
export class BitacoraAuditoriaComponent implements OnInit {
  ...
  private readonly service = inject(BitacoraAuditoriaService);
  ...
  guardar(): void {
    ...
    const datos = this.formulario.value as BitacoraAuditoria;
    if (this.editando()) {
  ...
  editar(item: BitacoraAuditoria): void {
    // ✅ signal.set() para actualizar estado de edición
    this.editando.set(true);
    this.idSeleccionado.set((item as any).idBitacora);
